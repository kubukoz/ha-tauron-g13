...
let TauronG13TimelineCard = class TauronG13TimelineCard extends i {
    setConfig(config) {
        this._config = config;
    }
    getCardSize() {
    ...
    render() {
        ...
        const tl = stateObj?.attributes?.timeline;
    ...
    _renderStrip(tl) {
        ...
        for (const seg of segments) {
            const x = seg.startIdx * w$1;
            const segW = (seg.endIdx - seg.startIdx + 1) * w$1;
        ...
        ${segW > 8
                ? w `<text x=${x + segW / 2} y="7.5" font-size="3.2"
      ...
      `);
            if (x - lastLabelX >= 9) {
                parts.push(w `<text x=${x} y="17" font-size="3" text-anchor="middle"
          fill="var(--secondary-text-color, #888)">${seg.startLabel}</text>`);
                lastLabelX = x;
            }
        }
        // Right-edge end label.
        parts.push(w `<text x="100" y="17" font-size="3" text-anchor="end"
      fill="var(--secondary-text-color, #888)">${hhmm(tl[n - 1].start)}</text>`);
        // Now marker.
        if (nowIdx >= 0) {
            const nx = (nowIdx + 0.5) * w$1;
            parts.push(w `
...
TauronG13TimelineCard.styles = i$3 `
    ha-card {
...
__decorate([
    n({ attribute: false })
], TauronG13TimelineCard.prototype, "hass", void 0);
__decorate([
    r()
], TauronG13TimelineCard.prototype, "_config", void 0);
TauronG13TimelineCard = __decorate([
    t("tauron-g13-timeline")
], TauronG13TimelineCard);
/** "2026-06-24T14:00:00+02:00" -> "14:00". */
function hhmm(iso) {
    return iso.slice(11, 16);
}
/** Local Date -> "YYYY-MM-DDTHH" to compare against an ISO entry's slice(0,13). */
function isoDateHour(d) {
...
(window.customCards).push({
    ...
    description: "Past + future G13 tariff zones as a merged-segment timeline bar.",
    preview: false,
});
// eslint-disable-next-line no-console
console.info("%c TAURON-G13-TIMELINE %c 0.3.0 ", "color:white;background:#2e7d32;font-weight:700", "color:#2e7d32;background:white");

export { TauronG13TimelineCard };
